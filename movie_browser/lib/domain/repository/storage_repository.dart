...
class StorageRepository implements IStorageRepository {
  ...
  late final Box<MovieSearchResponse> _movieResponseBox;
  ...
  Future<void> _onStart() async {
    ...
    _movieResponseBox = await Hive.openBox<MovieSearchResponse>('movies');
  ...
  void _listenToEventBusEvents() {
    ...
    _eventBus.on<SearchByTitleEvent>().listen((event) {
      if (!isOnline) {
        _eventBus.fire(MovieFullResultEvent(_movieResponseHiveManager
            .fetchData(event.search) as MovieSearchResponse));
      }
    });
